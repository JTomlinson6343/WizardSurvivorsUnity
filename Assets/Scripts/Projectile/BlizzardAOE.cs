...
public class BlizzardAOE : AOEObject
{
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("Enemy"))
        {
            if (!collision.gameObject.GetComponent<BlizzardDebuff>())
            {
                ...
                blizzardDebuff.Init(-1f, m_AbilitySource.GetTotalStats().damage, m_AbilitySource.m_Data.damageType,Player.m_Instance.gameObject,false,1,DebuffType.Blizzard);
